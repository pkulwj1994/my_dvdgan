        ...
        for step in range(start, self.total_step + 1):
            ...
            if step % self.sample_step == 0:
                ...
                for j in range(self.test_batch_size):
                    if self.use_tensorboard is True:
                        self.writer.add_image("Class_{}_No_{}_Step_{}".format(int(fixed_label[j].to('cpu')),j, step), make_grid(denorm(fake_videos[j].data)), step)
                    else:
                        save_image(denorm(fake_videos[j].data), os.path.join(self.sample_path, "Class_{}_No_{}_Step_{}.png".format(int(fixed_label[j].to('cpu')),j, step)))
